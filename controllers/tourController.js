...
exports.resizeTourImages = catchAsync(async (req, res, next) => {
  if (!req.files.imageCover || !req.files.images)
    return next(new AppError('You should upload images and image cover', 400));
  ...
  await Promise.all(
    req.files.images.map(async (el, i) => {
      const filename = `tour-${req.params.id}-${Date.now()}-cover-${i +
        1}.jpeg`;
  ...
  const plan = await Tour.aggregate([
    ...
    },
    {
      $group: {
        ...
        tours: { $push: '$name' }
        //create a new array tour with the names of the tours
      }
    },
    {
      $addFields: { month: '$_id' }
      //add new field and give names equal to _id
    },
    {
      $project: {
        _id: 0
        //hide _id field from results view
      }
    },
    {
      $sort: {
      ...
      // sort by num of tours in descending order in mounth
    }
    // {
    //   $limit: 6
    // }
